...
define(["require", "exports", '../../engine/Hex'], function (require, exports, Hex) {
    function run() {
        ...
        QUnit.test("CartesianCoordinate - FlatTop move only in Y direction", function (assert) {
            var cart = Hex.ToCartesianCoordinate({ a: 1, b: 0 }, 10, 0 /* FlatTop */);
            assert.equal(cart.x, 0);
            assert.equal(cart.y, Hex.CartesianHeight(10, 0 /* FlatTop */));
        });
        QUnit.test("CartesianCoordinate - FlatTop move only in X direction", function (assert) {
            var cart = Hex.ToCartesianCoordinate({ a: -1, b: 2 }, 10, 0 /* FlatTop */);
            assert.equal(cart.x, Hex.CartesianWidth(10, 0 /* FlatTop */) + 10);
