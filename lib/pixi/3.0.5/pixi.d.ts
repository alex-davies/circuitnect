...
declare module "lib/pixi/3.0.5/pixi" {

    //this d.ts file was originaly for pixi 2.x.x
    //belos is customizations to make it 3.0.0 friendly
    export module loaders{
        export class Loader{
            add(name: string, data:string): Loader;
            once(event:string, callback:()=>any): Loader;
            load():Loader;
        }
    }
    ...
    export module ticker{
        ...
        export interface Ticker{
            ...
            add(callback:(time:number)=>void, context?:any);
            addOnce(callback:(time:number)=>void, context?:any);
            remove(callback:(time:number)=>void, context?:any);
        }
    }
